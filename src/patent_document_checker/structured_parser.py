...
MAJOR_TAGS = {
    ...
    "発明の概要": "summary_of_invention",
    "図面の簡単な説明": "description_of_drawings",
    "発明を実施するための形態": "description_of_embodiments",
    "産業上の利用可能性": "industrial_applicability",
    "符号の説明": "reference_signes_list",
    "配列表フリーテキスト": "seequence_list_text",
    "配列表": "sequence_list",
    "要約": "abstract_tag",
}
...
SUBSECTION_TAGS = {
    "特許文献": "patcit",
    "非特許文献": "nplcit",
    "発明が解決しようとする課題": "tech_problem",
    "課題を解決するための手段": "tech_solution",
    "発明の効果": "advantageous-effects",
    "課題": "abstract_problem",
    "解決手段": "abstract_solution",
    "選択図": "abstract_selected_figure",
}
...
class PatentNode:
    ...
    def find_all(
        self, kind: str | None = None, tag_name: str | None = None
    ) -> list["PatentNode"]:
        matches: list[PatentNode] = []
        if (kind is None or self.kind == kind) and (
            tag_name is None or self.tag_name == tag_name
        ):
            matches.append(self)
...
def classify_tag(tag_name: str) -> str:
    ...
    if tag_name in MAJOR_TAGS:
        return MAJOR_TAGS[tag_name]
    if tag_name in SUBSECTION_TAGS:
        return SUBSECTION_TAGS[tag_name]
    if tag_name.startswith("実施例"):
        return "embodiments_example"
...
def tag_level(kind: str, stack: list[PatentNode]) -> int:
    ...
    if kind == "claim":
        return 2
    if kind in MAJOR_TAGS.values():
        return 2
    if kind == "application_container":
        return 2
    if kind in SUBSECTION_TAGS.values():
        return 3
    if kind == "application_field":
        return 3
    if kind == "paragraph":
        return 4
...
def _split_tagged_text(text: str) -> list[tuple[str | None, str]]:
    # Split one raw block into leading plain text and tagged segments.
    # Each tag owns the text up to the next tag so callers can build a tree node
    # without losing inline content such as "【請求項1】...【請求項2】...".
...
def _append_text(parent: PatentNode, text: str, block_index: int) -> None:
    ...
    parent.children.append(
        PatentNode(
            text=stripped, kind="text", level=parent.level + 1, block_index=block_index
        )
    )

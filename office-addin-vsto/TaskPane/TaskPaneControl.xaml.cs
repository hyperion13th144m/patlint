...
public partial class TaskPaneControl : UserControl
{
    ...
    private void SaveApiUrl_Click(object sender, RoutedEventArgs e)
    {
        ...
        SetStatus("API URLを保存しました。", isError: false);
    }
    ...
    {
        if (sender is Button btn && btn.Tag is int idx && idx < _lastDiagnostics.Count)
        {
    ...
    private UIElement BuildClaimsSection(List<ClaimView> claims)
    {
        ...
        foreach (var c in claims)
        {
            ...
            var inList = incoming.TryGetValue(c.Number, out var il) ? il : new List<int>();
